...
class PWCDCNet(nn.Module):
    ...
    def __init__(self, md=4):
        ...
        # 一步到原始尺寸
        self.deconv0 = deconv(2, 2, kernel_size=4, stride=4, padding=0)
    ...
    def forward(self,x):
        ...
        flow1 = self.deconv2(flow2)
        flow0 = self.deconv1(flow1)
        # flow0 = self.deconv0(flow2)
...
if __name__ == '__main__':
    ...
    pwc = PWCDCNet()
    pwc.to(device)
    # (Batch-Size, fixed and moving imgs, W, H)
    test_batch = torch.rand(8, 2, 256, 256).to(device)
    output = pwc(test_batch)
    print(len(output))
    print(output[0].size())
    #correlation.corr()
